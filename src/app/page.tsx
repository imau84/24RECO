...
function Tile({ icon, title, sub, href, color }: Item) {
  return (
    <a href={href} className="t" style={{ background: color }}>
      <span className="t-ic">{icon}</span>span>
    <span className="t-ti">{title}</span>span>
    <span className="t-su">{sub}</span>span>
    </a>a>
    )
}
...
function Band({ title, items }: { title: string; items: Item[] }) {
  return (
    ...
    <h2 className="band-t">{title}</h2>h2>
    <div className="grid">{items.map(i => <Tile key={i.title} {...i} />)}</div>div>
    </section>section>
    )
}
...
export default function HomePage() {
  return (
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    <div className="top">
    <span className="logo">24reco<span>.com</span>span></span>span>
    <nav className="nav"><a href="#industrii">Industrii</a>a><a href="#institutii">Instituții</a>a><a href="#rapoarte">Rapoarte</a>a></nav>nav>
    <Link href="/despre" className="sub-btn">Despre · Contact</Link>Link>
    </div>div>
    
    <div className="hero">
    <h1>Date publice. Pe înțelesul tuturor.</h1>h1>
    <p>Alege un domeniu și vezi cifrele care contează, explicate simplu.</p>p>
    </div>div>
    
    <div id="industrii"><Band title="Industrii" items={INDUSTRII} /></div>div>
    <div id="institutii"><Band title="Instituții Publice" items={INSTITUTII} /></div>div>
    <div id="rapoarte"><Band title="Rapoarte" items={RAPOARTE} /></div>div>
    
    <div className="foot">
    <span>24reco.com — Date publice din România</span>span>
    <span className="m">Surse: INS · BNR · MF · ME · ARR · AEP</span>span>
    </div>div>
    </main>main>
    )
}
